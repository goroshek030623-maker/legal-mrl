...
export default function CaseDetailPage() {
  ...
  const [isPaid, setIsPaid] = useState(false)  // TEST MODE: always show full document
  ...
  const getStatusBadge = (status: string) => {
    const styles: Record<string, string> = {
      new: 'bg-yellow-500/100/20 text-yellow-300',
  ...
  if (!caseData) return <div className="p-8 text-center text-red-400">Дело не найдено</div>
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      {/* Заголовок дела */}
      <div className="bg-slate-800/50 p-6 rounded-lg border">
      ...
      {/* Результат анализа */}
      {caseData.analysis && (
        <div className="bg-yellow-500/100/10 p-6 rounded-lg border border-blue-200">
          <h2 className="font-semibold text-white mb-2">Результат анализа</h2>
          <div className="text-yellow-300 whitespace-pre-wrap">{caseData.analysis}</div>
        </div>
      )}

      {/* Рекомендуемые действия */}
      {suggestedActions.length > 0 && (
        <div className="bg-slate-800/50 p-6 rounded-xl  border">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-8 h-8 rounded-full bg-emerald-500 text-slate-900 flex items-center justify-center font-bold text-sm">3</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {suggestedActions.map((action, index) => (
              <button
                ...
                className={`p-4 rounded-lg border text-left transition-all ${selectedAction === action ? 'bg-purple-50 border-purple-400 ring-2 ring-purple-200' : 'bg-slate-900 hover:bg-slate-700 border-slate-700'}`}
              >
      ...
      {generatedDoc && (
        <div className="bg-slate-800/50 p-6 rounded-lg border">
          <div className="flex justify-between items-start mb-4">
            ...
            {!isPaid && (
              <button
                ...
                disabled={isPaying}
                className="bg-emerald-500 text-slate-900 px-4 py-2 rounded-lg font-medium hover:bg-emerald-400 disabled:opacity-50"
              >
          ...
          <div className="bg-slate-800/50 p-8 rounded border-2 border-slate-700  whitespace-pre-wrap text-base font-serif leading-relaxed text-justify text-white min-h-[400px]">
            {isPaid ? (
              ...
              <>
                ...
                <div className="relative mt-4">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center z-20">
                    <div className="bg-slate-800/50 px-6 py-3 rounded-lg border shadow-lg text-center">
                      <p className="text-slate-300 font-medium mb-1">Продолжение доступно после оплаты</p>
                      <p className="text-sm text-slate-400">Сумма: 499 ₽</p>
                    </div>
                  </div>
                </div>
              </>
          ...
          {isPaid && (
            <div className="mt-4 flex justify-center">
              <button
                ...
                className="bg-yellow-500/100 text-white px-6 py-3 rounded-lg font-medium hover:bg-yellow-400 flex items-center gap-2"
              >
      ...
      <div className="bg-slate-800/50 p-6 rounded-lg border">
        ...
        <div className="mb-4">
          <input
            ...
            className="w-full px-3 py-2 border border-slate-700 rounded-lg backdrop-blur mb-2"
          ...
          {uploadFiles && uploadFiles.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="bg-yellow-500/100 text-white px-3 py-1 rounded text-sm hover:bg-yellow-400 disabled:opacity-50"
              >
      ...
      {showQrModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-slate-800/50 p-6 rounded-lg max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold mb-2">Оплата через СБП</h3>
            <p className="text-sm text-slate-300 mb-4">Сумма: 499 ₽</p>
            {paymentData?.paymentQrImages ? (
              <img src={paymentData.paymentQrImages.desktop || paymentData.paymentQrImages} alt="QR Code СБП" className="w-full mb-4 border bg-white p-4 rounded" />
            ) : (
              <img src="/images/sbp-qr.png" alt="QR Code СБП" className="w-full mb-4 border bg-white p-4 rounded" />
            )}
            <p className="text-xs text-slate-400 mb-4 text-center">Отсканируйте QR-код в приложении банка</p>
            {paymentData?.paymentUrl && (
              <a href={paymentData.paymentUrl} target="_blank" rel="noopener noreferrer" className="block text-yellow-400 underline mb-4 text-center">
                Открыть в банке
              </a>
            )}
            <div className="flex gap-2">
              {qrTimer > 0 ? (
                <button
                  disabled
                  className="flex-1 bg-gray-300 text-slate-400 py-2 rounded-lg font-medium cursor-not-allowed"
                >
                ...
                <button
                  ...
                  className="flex-1 bg-emerald-500 text-slate-900 py-2 rounded-lg font-medium hover:bg-emerald-400"
                >
              ...
              <button
                ...
                className="flex-1 bg-slate-600 text-slate-300 py-2 rounded-lg font-medium hover:bg-gray-300"
              >
                Закрыть
              </button>
            </div>
